...
Finally, let's plot the networks using the `gplot()` function. Note that we can use either the object `mat_d` which is of class `matrix` or the object `net_u` which is of class `network`.
